...
class mod(commands.Cog):
    ...
    async def clear(self, ctx, amount: int):
        if ctx.message.author.guild_permissions.manage_messages:
            ...
            embed = discord.Embed(
                title=f"`{amount}` messages were removed.",
                description="",
                color=0xFF0000,
            )
            ...
            embed = discord.Embed(
                title="Error:",
                description="Role must need manage messages to be enabled.",
                color=0xFF0000,
            )
    ...
    async def blacklist(self, ctx, user_id: int):
        ...
        try:
            ...
            await ctx.send(
                content=self.client.command_prefix + "Banned user: %s" % user_id
            )
        except discord.NotFound:
            await ctx.send(
                content=self.client.command_prefix + "Could not find user. "
                "Invalid user ID was provided."
            )
        except discord.errors.Forbidden:
            await ctx.send(
                content=self.client.command_prefix
